export class MergeksortedHelper {
    constructor() {}
    ...
    // Takes in an index since we want to use it for heapify When removing the top,
    // just pass in index 0
    bubbleDown(minHeap, index) {
        let currentIndex = index;
        let current = minHeap[currentIndex];
        let [leftIndex,
            rightIndex] = this.getChildIndices(currentIndex);
